...
DOCKER_PREFIX=$MANIFEST_REGISTRY PULL_POLICY=$PULL_POLICY KUBEVIRT_API_LIFECYCLE_AUTOMATION_NAMESPACE=${KUBEVIRT_API_LIFECYCLE_AUTOMATION_NAMESPACE} TARGET_NAMESPACE=${TARGET_NAMESPACE} RESTART_REQUIRED=${RESTART_REQUIRED} MACHINE_TYPE_GLOBAL=${MACHINE_TYPE_GLOBAL} LABEL_SELECTOR=${LABEL_SELECTOR} make manifests
