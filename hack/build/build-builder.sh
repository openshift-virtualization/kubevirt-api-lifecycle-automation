...
#Copyright 2023 The KubevirtApiLifecycleAutomation Authors.
#
...
function build_and_push() {
  if [ "${KUBEVIRT_API_LIFECYCLE_AUTOMATION_CRI}" = "docker" ]; then
     KUBEVIRT_API_LIFECYCLE_AUTOMATION_CONTAINER_BUILDCMD=${KUBEVIRT_API_LIFECYCLE_AUTOMATION_CONTAINER_BUILDCMD:-docker}
  else
     KUBEVIRT_API_LIFECYCLE_AUTOMATION_CONTAINER_BUILDCMD=${KUBEVIRT_API_LIFECYCLE_AUTOMATION_CONTAINER_BUILDCMD:-buildah}
  fi

  if [ "${KUBEVIRT_API_LIFECYCLE_AUTOMATION_CONTAINER_BUILDCMD}" = "buildah" ]; then
  ...
  UNTAGGED_BUILDER_IMAGE=quay.io/bmordeha/kubevirt-api-lifecycle-automation-builder
  ...
  DIGEST=$(${KUBEVIRT_API_LIFECYCLE_AUTOMATION_CRI} images --digests | grep ${UNTAGGED_BUILDER_IMAGE} | grep ${BUILDER_TAG} | awk '{ print $4 }')
