...
cluster-sync: cluster-clean-kubevirt-api-lifecycle-automation
	./cluster-sync/sync.sh KUBEVIRT_API_LIFECYCLE_AUTOMATION_AVAILABLE_TIMEOUT=${KUBEVIRT_API_LIFECYCLE_AUTOMATION_AVAILABLE_TIMEOUT} DOCKER_PREFIX=${DOCKER_PREFIX} DOCKER_TAG=${DOCKER_TAG} PULL_POLICY=${PULL_POLICY} KUBEVIRT_API_LIFECYCLE_AUTOMATION_NAMESPACE=${KUBEVIRT_API_LIFECYCLE_AUTOMATION_NAMESPACE} TARGET_NAMESPACE=${TARGET_NAMESPACE} RESTART_REQUIRED=${RESTART_REQUIRED} MACHINE_TYPE_GLOBAL=${MACHINE_TYPE_GLOBAL} LABEL_SELECTOR=${LABEL_SELECTOR}
